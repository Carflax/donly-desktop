...
export default function LabelPreview({ data, width, height, dpi, onUpdateElement, selectedId, onSelectElement, onRemoveElement, onInteractionEnd }: { data: LabelData, width: number, height: number, dpi: number, onUpdateElement?: (id: string, updates: Partial<LabelElement>) => void, selectedId?: string | null, onSelectElement?: (id: string | null) => void, onRemoveElement?: (id: string) => void, onInteractionEnd?: () => void }) {
  ...
  const [contextMenu, setContextMenu] = useState<{ x: number, y: number, visible: boolean, itemId: string | null }>({ x: 0, y: 0, visible: false, itemId: null });
  const [imagesCache, setImagesCache] = useState<Record<string, HTMLImageElement>>({});

  const px = (mm: number) => (mm * dpi) / 25.4;

  useEffect(() => {
    const handleGlobalClick = () => setContextMenu(p => p.visible ? { ...p, visible: false } : p);
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    const mmX = ((e.clientX - rect.left) * (canvas.width / rect.width) * 25.4) / dpi;
    const mmY = ((e.clientY - rect.top) * (canvas.height / rect.height) * 25.4) / dpi;
    if (selectedId) {
       const elements = data.custom[data.activeTab] || []; const el = elements.find(item => item.id === selectedId);
       if (el) {
          const w = el.type === "text" ? (el.content.length * (el.fontSize || 4) * 0.5) : (el.w || 20);
          const h = el.type === "text" ? (el.fontSize || 4) : (el.h || 10);
          const cx = el.type === "text" ? el.x : el.x + w/2; const cy = el.type === "text" ? el.y - h/2 : el.y + h/2;
          const rad = ((el.rotation || 0) * Math.PI) / 180;
          const dx = mmX - cx; const dy = mmY - cy;
          const localX = dx * Math.cos(-rad) - dy * Math.sin(-rad); const localY = dx * Math.sin(-rad) + dy * Math.cos(-rad);
          const h_mm = 20 * 25.4 / dpi;
          if (Math.abs(localX - 0) < 5 && Math.abs(localY - (-h/2 - h_mm)) < 5) { rotatingId.current = el.id; rotateStart.current = { cx, cy }; return; }
          const delLX = w/2 + (4 * 25.4 / dpi); const delLY = -h/2 - (4 * 25.4 / dpi);
          if (Math.abs(localX - delLX) < 5 && Math.abs(localY - delLY) < 5) { onRemoveElement?.(el.id); return; }
          if (Math.abs(localX - w/2) < 5 && Math.abs(localY - h/2) < 5) { resizingId.current = el.id; resizeStart.current = { mmX, mmY, w: el.w || 20, h: el.h || 10, fontSize: el.fontSize || 4 }; return; }
       }
    }
    const elements = (data.custom && data.custom[data.activeTab]) || [];
    let hit = false;
    for (let i = elements.length - 1; i >= 0; i--) {
      ...
      const w = el.type === "text" ? (el.content.length * (el.fontSize || 4) * 0.5) : (el.w || 20);
      const h = el.type === "text" ? (el.fontSize || 4) : (el.h || 10);
      const isInsideX = el.type === "text" ? (mmX >= el.x - w/2 && mmX <= el.x + w/2) : (mmX >= el.x && mmX <= el.x + w);
      const isInsideY = el.type === "text" ? (mmY >= el.y - h && mmY <= el.y) : (mmY >= el.y && mmY <= el.y + h);
      if (isInsideX && isInsideY) { draggingId.current = el.id; dragStart.current = { x: mmX, y: mmY, elX: el.x, elY: el.y }; onSelectElement?.(el.id); hit = true; break; }
    }
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    const mmX = ((e.clientX - rect.left) * (canvas.width / rect.width) * 25.4) / dpi;
    const mmY = ((e.clientY - rect.top) * (canvas.height / rect.height) * 25.4) / dpi;
    if (rotatingId.current && onUpdateElement) { const dx = mmX - rotateStart.current.cx; const dy = mmY - rotateStart.current.cy; let deg = Math.atan2(dy, dx) * 180 / Math.PI + 90; onUpdateElement(rotatingId.current, { rotation: Math.round(deg) }); return; }
    if (resizingId.current && onUpdateElement) {
       const dx = mmX - resizeStart.current.mmX; const dy = mmY - resizeStart.current.mmY;
       const elements = data.custom[data.activeTab] || []; const el = elements.find(item => item.id === resizingId.current);
       if (el) {
          if (el.type === "text") { onUpdateElement(el.id, { fontSize: Number(Math.max(2, resizeStart.current.fontSize + dy/2).toFixed(1)) }); }
          else { let newW = resizeStart.current.w + dx; let newH = resizeStart.current.h + dy; if (e.shiftKey) { const ratio = resizeStart.current.w / resizeStart.current.h; newW = newH * ratio; } onUpdateElement(el.id, { w: Math.max(5, newW), h: Math.max(2, newH) }); }
       }
       return;
    }
    if (!draggingId.current || !onUpdateElement) return;
    const dx = mmX - dragStart.current.x; const dy = mmY - dragStart.current.y;
    let fx = dragStart.current.elX + dx; let fy = dragStart.current.elY + dy;
    const snap = 2; const cx = width / 2; const cy = height / 2;
    const el = data.custom[data.activeTab]?.find(item => item.id === draggingId.current);
    const g: { x?: number, y?: number } = {};
    if (el) {
       const w = el.type === "text" ? (el.content.length * (el.fontSize || 4) * 0.5) : (el.w || 20);
       const h = el.type === "text" ? (el.fontSize || 4) : (el.h || 10);
       const ecx = el.type === "text" ? fx : fx + w / 2; const ecy = el.type === "text" ? fy - h / 2 : fy + h / 2;
       if (Math.abs(ecx - cx) < snap) { fx = el.type === "text" ? cx : cx - w / 2; g.x = cx; }
       if (Math.abs(ecy - cy) < snap) { fy = el.type === "text" ? cy + h / 2 : cy - h / 2; g.y = cy; }
    }
    setGuides(g); onUpdateElement(draggingId.current, { x: Math.round(fx), y: Math.round(fy) });
  };

  const handleMouseUp = () => { if (draggingId.current || resizingId.current || rotatingId.current) onInteractionEnd?.(); draggingId.current = null; resizingId.current = null; rotatingId.current = null; setGuides({}); };

  return (
    <div className="relative group/canvas">
      <canvas ref={canvasRef} width={px(width)} height={px(height)} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp}
        onDoubleClick={(e) => {
           const canvas = canvasRef.current; if (!canvas) return; const rect = canvas.getBoundingClientRect();
           const mmX = ((e.clientX - rect.left) * (canvas.width / rect.width) * 25.4) / dpi;
           const mmY = ((e.clientY - rect.top) * (canvas.height / rect.height) * 25.4) / dpi;
           const el = data.custom[data.activeTab]?.find(it => { if (it.type !== "text") return false; const w = it.content.length * (it.fontSize || 4) * 0.5; const h = it.fontSize || 4; return mmX >= it.x - w/2 && mmX <= it.x + w/2 && mmY >= it.y - h && mmY <= it.y; });
           if (el) { setEditingId(el.id); setEditValue(el.content); }
        }}
        onContextMenu={(e) => {
           e.preventDefault(); const canvas = canvasRef.current; if (!canvas) return; const rect = canvas.getBoundingClientRect();
           const mmX = ((e.clientX - rect.left) * (canvas.width / rect.width) * 25.4) / dpi;
           const mmY = ((e.clientY - rect.top) * (canvas.height / rect.height) * 25.4) / dpi;
           const el = [...(data.custom[data.activeTab] || [])].reverse().find(it => { const w = it.type === "text" ? (it.content.length * (it.fontSize || 4) * 0.5) : (it.w || 20); const h = it.type === "text" ? (it.fontSize || 4) : (it.h || 10); return it.type === "text" ? (mmX >= it.x - w/2 && mmX <= it.x + w/2 && mmY >= it.y - h && mmY <= it.y) : (mmX >= it.x && mmX <= it.x + w && mmY >= it.y && mmY <= it.y + h); });
           if (el) setContextMenu({ x: e.clientX, y: e.clientY, visible: true, itemId: el.id });
        }}
        style={{ cursor: rotatingId.current ? "crosshair" : (resizingId.current ? "nwse-resize" : (draggingId.current ? "grabbing" : "crosshair")) }}
        className="max-w-full h-auto rounded border border-slate-300 bg-white shadow-2xl"
      />
      {editingId && (
         <input autoFocus value={editValue} onChange={(e) => setEditValue(e.target.value)} onBlur={() => { if (editingId) onUpdateElement?.(editingId, { content: editValue }); setEditingId(null); }} onKeyDown={(e) => e.key === "Enter" && (editingId && onUpdateElement?.(editingId, { content: editValue }), setEditingId(null))}
           className="absolute bg-white/80 border border-accent/30 rounded px-1 outline-none text-black text-center"
           style={{ left: `${(data.custom[data.activeTab]?.find(e => e.id === editingId)!.x / width) * 100}%`, bottom: `${((height - data.custom[data.activeTab]?.find(e => e.id === editingId)!.y) / height) * 100}%`, fontSize: `${px(data.custom[data.activeTab]?.find(e => e.id === editingId)!.fontSize || 4) * (canvasRef.current?.getBoundingClientRect().width || 1) / (canvasRef.current?.width || 1)}px`, lineHeight: 1, transform: "translate(-50%, 5px)" }}
         />
      )}
      {contextMenu.visible && (
         <div className="fixed z-[100] bg-slate-900/90 backdrop-blur-xl border border-white/20 rounded-xl py-2 shadow-2xl min-w-[120px]" style={{ left: contextMenu.x, top: contextMenu.y }}>
            <button onClick={() => { contextMenu.itemId && onRemoveElement?.(contextMenu.itemId); setContextMenu(p => ({ ...p, visible: false })); }} className="w-full text-left px-4 py-2 text-white hover:bg-red-500/80 text-xs font-medium">Excluir</button>
         </div>
      )}
    </div>
  );
